 ...
 * Uses local SQLite via Drizzle ORM (expo-sqlite async API — works on web too).
 */
...
export function useAllTransactions() {
    ...
    const refresh = useCallback(async () => {
        ...
        try {
            const db = await dbPromise;
            setData(await db.select().from(transactions).orderBy(desc(transactions.date)));
        } catch (err) {
...
export function useSummary(filters: { month?: number; year?: number } = {}) {
    ...
    const refresh = useCallback(async () => {
        ...
        try {
            ...
            setData({ balance: totalIncome - totalExpenses, totalIncome, totalExpenses, month, year });
        } catch (err) {
